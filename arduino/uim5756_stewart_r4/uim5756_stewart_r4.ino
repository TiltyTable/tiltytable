/*
  Full-rotation Stewart executor (Arduino Uno R4 WiFi)
...
#error "Select Arduino UNO R4 WiFi (arduino:renesas_uno:unor4wifi) for this sketch."
#endif
...
// MobaTools drives all three axes from an RA4M1 GPT timer. Keep the validated
// motion envelope until motor/load tests show that the mechanics can go faster.
...
// RA4M1 reset flags survive into setup(). An external RESET-pin event has no
// unsafe flag. Power/voltage, watchdog, software and fault resets do, and must
// never restore open-loop motor coordinates automatically.
uint8_t resetStatus0 = 0;
uint16_t resetStatus1 = 0;

void captureResetStatus() {
  resetStatus0 = R_SYSTEM->RSTSR0;
  resetStatus1 = R_SYSTEM->RSTSR1;
  // Status flags are cumulative until cleared. Preserve this boot's snapshot,
  // then clear them so a later RESET-pin event can be recognized safely.
  R_SYSTEM->RSTSR0 = 0;
  R_SYSTEM->RSTSR1 = 0;
}
...
MoToStepper stepper0((long)STEPS_PER_CRANK_REV, STEPDIR);
MoToStepper stepper1((long)STEPS_PER_CRANK_REV, STEPDIR);
MoToStepper stepper2((long)STEPS_PER_CRANK_REV, STEPDIR);
MoToStepper *stepper[AXES] = {&stepper0, &stepper1, &stepper2};
...
void setAxisEnable(uint8_t axis, bool enabled) {
  // MobaTools continues stepping from its timer even when the external driver
  // is disabled. Its immediate stop completes on the next timer pulse, so wait
  // for that pulse before dropping ENA and reading the final coordinate.
  if (!enabled) {
    stepper[axis]->stop();
    while (stepper[axis]->moving()) {}
...
bool moving() {
  for (uint8_t axis = 0; axis < AXES; axis++) {
    if (axisEnabled[axis] && stepper[axis]->moving()) return true;
  }
  return false;
}
...
bool savePersisted() {
  ...
  for (uint8_t axis = 0; axis < AXES; axis++) {
    state.steps[axis] = stepper[axis]->readSteps();
  }
...
bool loadPersisted() {
  ...
  const uint8_t unsafeStatus0 =
    R_SYSTEM_RSTSR0_PORF_Msk |
    R_SYSTEM_RSTSR0_LVD0RF_Msk |
    R_SYSTEM_RSTSR0_LVD1RF_Msk |
    R_SYSTEM_RSTSR0_LVD2RF_Msk |
    R_SYSTEM_RSTSR0_DPSRSTF_Msk;
  const uint16_t unsafeStatus1 =
    R_SYSTEM_RSTSR1_IWDTRF_Msk |
    R_SYSTEM_RSTSR1_WDTRF_Msk |
    R_SYSTEM_RSTSR1_SWRF_Msk |
    R_SYSTEM_RSTSR1_RPERF_Msk |
    R_SYSTEM_RSTSR1_REERF_Msk |
    R_SYSTEM_RSTSR1_BUSSRF_Msk |
    R_SYSTEM_RSTSR1_BUSMRF_Msk |
    R_SYSTEM_RSTSR1_SPERF_Msk |
    R_SYSTEM_RSTSR1_TZERF_Msk |
    R_SYSTEM_RSTSR1_CPERF_Msk;
  bool unsafeReset =
    (resetStatus0 & unsafeStatus0) != 0 ||
    (resetStatus1 & unsafeStatus1) != 0;
  bool externalReset = resetStatus0 == 0 && resetStatus1 == 0;
  if (!externalReset || unsafeReset) return false;

  for (uint8_t axis = 0; axis < AXES; axis++) {
    stepper[axis]->setZero(-state.steps[axis]);
...
void setMotionProfile(uint8_t axis, float speedSteps, float accelSteps) {
  if (!profileValueChanged(configuredSpeedSteps[axis], speedSteps) &&
      !profileValueChanged(configuredAccelSteps[axis], accelSteps)) return;

  // MobaTools specifies speed in steps/10 seconds and acceleration as the
  // number of steps from rest to target speed. Approximate the requested
  // physical acceleration with the constant-acceleration stopping distance.
  uint32_t speed10 = max(1UL, (uint32_t)lround(speedSteps * 10.0));
  uint32_t rampSteps = constrain(
    (uint32_t)lround(speedSteps * speedSteps / (2.0 * accelSteps)),
    1UL,
    (uint32_t)MAXRAMPLEN
  );
  stepper[axis]->setSpeedSteps(speed10, rampSteps);
...
void applyCoordinatedTargets(const long target[AXES]) {
  long maxDelta = 0;
  for (uint8_t axis = 0; axis < AXES; axis++) {
    long delta = labs(target[axis] - stepper[axis]->readSteps());
...
void printStatus() {
  ...
  for (uint8_t axis = 0; axis < AXES; axis++) {
    ...
    Serial.print(stepper[axis]->readSteps());
  ...
  Serial.print(F(" backend=moba_gpt ready="));
  Serial.print(motorBackendReady ? 1 : 0);
  Serial.print(F(" vmax=")); Serial.print(profileSpeedDegS, 3);
  Serial.print(F(" amax=")); Serial.println(profileAccelDegS2, 3);
}

void printHelp() {
  Serial.println(F("UIM5756PM STEWART R4 v1"));
...
void handleCommand(String command) {
  ...
  if (!motorBackendReady && first != "EXP?" && first != "HELP" &&
      first != "?" && first != "STATUS") {
    Serial.println(F("ERR MOTOR_BACKEND"));
    return;
  }
  ...
  if (first == "EXP?") {
    ...
    if (action == "BEGIN") {
      ...
      desiredTarget[axis] = stepper[axis]->readSteps() + pulses;
...
void setup() {
  ...
  for (uint8_t axis = 0; axis < AXES; axis++) {
    ...
    digitalWrite(ENA_PIN[axis], logicLevel(false, ENA_ACTIVE_LOW));
    axisEnabled[axis] = false;
  }

  motorBackendReady = true;
  for (uint8_t axis = 0; axis < AXES; axis++) {
    uint8_t invert = INV_STEP;
    if (DIR_INVERT[axis]) invert |= INV_DIR;
    if (!stepper[axis]->attach(PLS_PIN[axis], DIR_PIN[axis], invert)) {
      motorBackendReady = false;
    }
    stepper[axis]->setZero();
    desiredTarget[axis] = 0;
  }

  if (motorBackendReady) {
    for (uint8_t axis = 0; axis < AXES; axis++) {
      setMotionProfile(
        axis,
        profileSpeedDegS * STEPS_PER_CRANK_REV / 360.0,
  ...
  setEnable(false);
  bool didRestore = motorBackendReady && loadPersisted();
  Serial.println(F("UIM5756PM STEWART R4 ready; DISABLED."));
  if (!motorBackendReady) {
    Serial.println(F("FATAL MobaTools attach failed; motion locked."));
  }
  Serial.println(
    didRestore
...
void loop() {
  uint8_t bytesRead = 0;
  while (Serial.available() && bytesRead < 32) {
